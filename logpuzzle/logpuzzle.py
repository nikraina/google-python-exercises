...
def url_sort(url_str):
  url_string = re.search(r'-(\w+).jpg', url_str)
  if url_string:
  	  return url_string.group(1)
  else:
  	  return url_string
...
def read_urls(filename):
  ...
  host = filename[under_index+1 :]
  
  lcl_file = open(filename, "rU")
  url_dict={}
  for line in lcl_file:
  	  url_line = re.search(r"GET (\S+puzzle\S+)", line)
  	  if url_line:
  	  	  url_dict["http://" + host + url_line.group(1)] = 1
